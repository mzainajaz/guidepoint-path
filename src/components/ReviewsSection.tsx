...
const ReviewCard = ({ review, index }: { review: Review; index: number }) => (
  <motion.div
    ...
    className="relative bg-white border border-border rounded-2xl p-6 flex flex-col gap-4 hover:shadow-lg hover:border-amber-400/30 transition-all duration-300"
  >
    {/* Quote icon */}
    <Quote className="h-5 w-5 text-amber-400/50 flex-shrink-0" />
    ...
    <div className="flex gap-0.5">
      {Array.from({ length: 5 }).map((_, i) => (
        <Star
          key={i}
          className={`h-3.5 w-3.5 ${i < review.rating ? "text-amber-400 fill-amber-400" : "text-border"}`}
        />
      ))}
    </div>
    ...
    <div className="flex items-center justify-between pt-3 border-t border-border">
      <div className="flex items-center gap-3">
        ...
        <div>
          <p className="text-foreground text-sm font-semibold leading-tight">{review.name}</p>
          <p className="text-muted-foreground text-xs mt-0.5">{review.role} · {review.country}</p>
        </div>
      </div>
      <div className="text-right">
        <SourceBadge source={review.source} />
        <p className="text-[10px] text-muted-foreground/60 mt-0.5">{review.date}</p>
      </div>
    </div>
  </motion.div>
);
  ...
  return (
    <section className="py-20 md:py-28 bg-secondary/50">
      <div className="container">
        ...
        >
          <div>
            <span className="badge-gold mb-3">Customer Reviews</span>
            <h2 className="font-display text-3xl md:text-4xl font-bold text-foreground leading-tight tracking-tight mt-4">
              Trusted by founders worldwide
            </h2>
            <p className="text-muted-foreground mt-4 text-base max-w-md leading-relaxed">
              Hundreds of entrepreneurs have used GuidePoint to navigate their UAE business setup with confidence.
            </p>
          </div>

          {/* Aggregate rating */}
          <div className="flex items-center gap-4 bg-white border border-border rounded-2xl px-6 py-4 flex-shrink-0 shadow-sm">
            <div className="text-center">
              <div className="font-display text-4xl font-bold text-foreground">{avgRating}</div>
              <div className="flex gap-0.5 mt-1 justify-center">
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star key={i} className="h-3 w-3 text-amber-400 fill-amber-400" />
                ))}
              </div>
              <div className="text-muted-foreground text-xs mt-1">{reviews.length} reviews</div>
            </div>
            <div className="w-px h-12 bg-border" />
            <div className="text-center">
              <div className="font-display text-4xl font-bold text-emerald-600">98%</div>
              <div className="text-muted-foreground text-xs mt-1 leading-snug">would<br/>recommend</div>
            </div>
          </div>
        </motion.div>
