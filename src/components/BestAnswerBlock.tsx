...
const BestAnswerBlock = ({ title, content, audience, caution }: BestAnswerBlockProps) => {
  const { t } = useLocale();
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="relative rounded-2xl p-6 md:p-8 border border-accent/20 bg-accent/5 overflow-hidden">
