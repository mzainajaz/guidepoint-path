      ...
      <div className="container">
        ...
        <div className="flex flex-wrap justify-center gap-4">
          {t.countryCards.countries.map((country, i) => (
            <motion.div key={country.code} initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.06 }}>
              <Link to={lp(`/relocation/${country.code}`)} className="group flex items-center gap-4 bg-card border border-border rounded-2xl px-7 py-5 hover:border-accent/30 hover:shadow-lg transition-all duration-300 min-w-[200px]">
